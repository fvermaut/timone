...
export function askCheckPrompt(input: AskCheckInput): string {
  return [
    ...
    "",
    "**You are writing to the person, and only to the person.** You cannot",
    "talk to the machine, and nothing you write goes back to whoever composed",
    "that message. So if what is wrong is the message itself — it points at a",
    "question that is not there, it is unclear, it leaves something out — that",
    "is not yours to fix and there is nobody there to fix it. Answer `LET IT",
    "THROUGH`. A question the person cannot answer is worse than the message",
    "you replaced, because the message at least told them what to do.",
    "",
    "Rules for a question you write:",
    "",
    "- Write it to them, in the second person, and make it answerable by",
    "  someone who has read only this ticket.",
