...
function deliveryPrompt(context: PromptContext): string {
  ...
  return [
    ...
    "**Never merge the pull request** — merging it is the human's act, and the",
    "pull request exists to let them take it.",
    "",
...
function breakdownPrompt(context: PromptContext): string {
  ...
  return [
    ...
    reentryBlock(),
    "",
    `**Work on the branch \`${branch ?? "the run's work branch"}\`** — cut it from`,
    "the project's default branch if it does not exist yet, and do all of this",
    "stage's work there. Nothing goes on the default branch.",
    "",
    "**The approved specification is committed on that branch** — either the",
    "stage before this one wrote it there, or it merged long ago and the branch",
    "carries it. It is what you are breaking up: read it, and do not re-derive",
    "it from this ticket.",
    "",
...
function planningPrompt(context: PromptContext): string {
  ...
  return [
    ...
    "stage's work there. Nothing goes on the default branch.",
    "",
    "**The approved requirements are committed on that branch** — either an",
    "earlier stage wrote them there, or they merged with the list of pieces and",
    "the branch carries them. They are what you are planning against: read",
    "them, and do not re-derive them from this ticket.",
    "",
