...
> ✏ Amended 2026-09-04: the *Reading a screen of figures* rules are traced to a primary source where a defensible one was found, and marked house style where none was, closing [timone#39](https://github.com/fvermaut/timone/issues/39).
...
- **A column is as wide as its widest figure, plus its padding. Never stretch a table to fill the viewport.** Stretching puts empty space between a name and its number, and comparing down a column is what a table is for — table columns are sized to the data they hold, not to the page, and spreading a table to fill the width detaches its cells from their neighbours ([A List Apart, Web Typography: Designing Tables to be Read, Not Looked At](https://alistapart.com/article/web-typography-tables/)). Wide content scrolls in its own container — which the responsive posture already requires — rather than being spread to fit.
- **Figures that are compared vertically are set tabular** (`font-variant-numeric: tabular-nums`, [MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/font-variant-numeric)), right-aligned, and keep their trailing zeros. Proportional digits make a column of numbers ragged at the decimal point and defeat scanning: tabular figures exist for exactly one purpose, vertically aligned columns, and using them is what makes scanning a large table of numbers fast and less error-prone ([Butterick's Practical Typography, Grids of Numbers](https://practicaltypography.com/grids-of-numbers.html) — the MDN link above documents the CSS property, this is the reason to reach for it).
- **Density is a stated decision, not an accident.** A screen built for scanning many rows declares its row height and offers the operator the looser alternative; a screen built for reading one thing does not need the choice. Either way the number is chosen, not inherited from a component library's default padding. No outside source makes this exact claim — a stated density decision paired with an operator-visible looser alternative — so the rule stands as this product family's house style, not a cited finding.
- **A screen has an explicit ground and a small number of foreground levels.** Primary figure, supporting label, and dimmed or absent value should be distinguishable without reading them — the same property NN/g's squint test checks for, where blurring a design past legibility should still show what matters most ([NN/g, Visual Hierarchy in UX: Definition](https://www.nngroup.com/articles/visual-hierarchy-ux-definition/)). That levels must be distinguishable without reading is sourced; the count — three levels is usually enough, more than four is noise — is house style.
...
- **A repeated visual element means one thing.** The same mark used for two facts on one screen is a defect, however convenient — it is the visual-design form of the standing rule that a user should never have to wonder whether the same thing means the same thing twice ([NN/g, 10 Usability Heuristics for User Interface Design — Consistency and standards](https://www.nngroup.com/articles/ten-usability-heuristics/)), and it was found and fixed in `ivtrends`' own prototype for that reason.
...
- NN/g — [Visual Hierarchy in UX: Definition](https://www.nngroup.com/articles/visual-hierarchy-ux-definition/) · [10 Usability Heuristics for User Interface Design](https://www.nngroup.com/articles/ten-usability-heuristics/)
- A List Apart — [Web Typography: Designing Tables to be Read, Not Looked At](https://alistapart.com/article/web-typography-tables/)
- Butterick's Practical Typography — [Grids of Numbers](https://practicaltypography.com/grids-of-numbers.html)
