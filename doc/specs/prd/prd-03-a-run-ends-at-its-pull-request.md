...
**The pull request opens on what was bent.** The first thing the body says is what changed on the way and what could not be checked (R2): plan amendments, requirement amendments, checks not run, workarounds, a failing state if there is one — or that nothing was bent. Merging accepts every listed item. Closing the pull request without merging rejects the work whole: nothing is patched in place, nothing is committed further to the branch, and the requirements on the default branch are unchanged. The run does not restart on its own — a close by hand is ambiguous, so the machine stops, marks the ticket held, and asks whether the work was unwanted, should be built again, or was closed by mistake (✏ 2026-09-07, [timone#111](https://github.com/fvermaut/timone/issues/111): this was the automatic-re-entry model until the phase-35 live gate found the code stops and asks instead, and fvermaut chose to keep the stop-and-ask).
