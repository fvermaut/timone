...
- **One branch from ticket to pull request.** The run's work branch — `timone/6-…`, claimed at the requirements stage — carries the PRD, the plan, the code, the reports, and becomes the PR head. R7 says the PR comes "from the work branch", and splitting execution onto a second branch would divorce the code from the requirements it implements. ⚠ **This diverges from `process.md` stage 6**, which names the branch `phase-NN-<slug>` and cuts it from the default branch — a rule written for the hand-run path, before runs owned branches. Built to R7, as the later and more specific decision; the divergence is **flagged for a grill, not resolved here** (the 12e precedent — the same move that produced ADR-0014). The execution prompt says "stay on this branch" explicitly, so the skill's branch rule is overridden by instruction, not silently contradicted.
...
- **A change-requesting review comment is confirmed intake.** Stage 9's confirmation gate exists so the machine never acts on its own diagnosis; a review comment *is* the human naming the change themselves, and asking them to confirm what they just asked for would be the form-over-conversation failure this project keeps refusing. Ambiguity routes to a clarifying reply in-thread, never to a guess. ⚠ **The remediation itself diverges from stage 9's letter** ("stage 9 never commits application code") — built to R11, the later and more specific decision, with the divergence **flagged for the same grill** as the branch rule. The cycle reuses this phase's own machinery on the same branch — a fix session, then re-verification, then the PR refreshed as an iteration — so nothing lands on the PR unverified, which is the invariant stage 9's rule protects.
...
- [ ] The prompt overrides the skill's branch rule by instruction, not by silence
...
**[NEW FILE]** `doc/plans/phases/reports/phase-13-complete.md` — including the two flagged process divergences (stage 6's branch rule, stage 9's no-code rule vs R11) carried to the open questions for their grill.
...
- [ ] Both process divergences are recorded as needing a grill, not smoothed over
