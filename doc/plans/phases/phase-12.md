...
> **Status:** Awaiting approval. Hand-planned 2026-08-03, as all Timone-self phases are (`/timone-plan` targets managed projects only); the plan skill's shape rules — thin vertical slices, declared seams, per-slice validation — are followed, not the instrument.
...
- **What holds a project, restated.** ✏ **This changes phase 11's rule and is the one decision in this plan that needs fvermaut's active agreement rather than his signature.** Today any parked run holds its project, so with phase 11 alone `scratch-app` is frozen behind one unanswered ticket. Proposed instead: **one *running session* per project at all times** (unchanged — sessions serialize), but **a parked run holds the project only once it owns a work branch**. Triage and clarification touch no repo, so several tickets may sit parked awaiting answers at once; from the requirements stage onward a run owns a branch and holds the project until it reaches a terminal state. *Alternative, if preferred:* keep phase 11's simpler rule — everything parked holds the project — and accept that one unanswered question freezes the project. Say which; the slices differ by roughly ten lines.
...
- **Housekeeping, not a slice:** `scratch-app-2` and `scratch-existing` declare local fixture paths as `repo_url`, so every poll logs two adapter refusals. One line in the poll loop skips non-GitHub projects quietly, or they leave the manifest. Fold into 12b if wanted; say which.
