...
> **Twelfth phase of [PRD-02](../../specs/prd/prd-02-inversion-of-control.md), and the half [phase 22](phase-22.md) could not build.** Governing decision: **[ADR-0030](../../adr/0030-the-breakdown-is-a-stage-and-chunk-zero-merges-without-a-pull-request.md)** — the breakdown is a pipeline stage of its own, and chunk zero merges without a pull request. **That record is `proposed`, and it is the only one in `doc/adr/` that is. Approving this phase is what accepts it**, exactly as fvermaut's approval of [phase 22](phase-22.md) was the ruling [ADR-0028](../../adr/0028-the-breakdown-is-an-artifact-and-the-ticket-follows-it.md) recorded. Until then nothing in ADR-0030 is normative and the mechanism is still open. Standing: [ADR-0026](../../adr/0026-a-ticket-is-a-conversation-a-run-is-a-chunk.md), [ADR-0028](../../adr/0028-the-breakdown-is-an-artifact-and-the-ticket-follows-it.md), [ADR-0029](../../adr/0029-a-chunk-advances-only-on-success.md), [ADR-0012](../../adr/0012-conversation-channels.md), [ADR-0014](../../adr/0014-artifact-first-gates.md), [ADR-0015](../../adr/0015-branch-per-driving-unit.md), [ADR-0016](../../adr/0016-review-remediation-rides-the-verify-fix-shape.md), [ADR-0019](../../adr/0019-timone-authored-commits-carry-a-provenance-trailer.md).
...
- **One gate for a whole initiative is the point, and it is the risk.** After this, a five-chunk milestone is approved once and judged five times as pull requests. [ADR-0030](../../adr/0030-the-breakdown-is-a-stage-and-chunk-zero-merges-without-a-pull-request.md) says this is where that choice stops being cheap to reverse. 23i's human gate is where fvermaut says whether it is the rhythm he ruled for.
...
**Doneness is derived, not written into the file — and this is a deliberate departure from R22 clause 5's literal wording, flagged for fvermaut at approval.** The clause says *"the merged chunk is marked done in the breakdown"*. Marking it there means the **poll loop** editing, committing and pushing a file on a client's default branch, and the daemon has never committed or pushed anything anywhere — `src/git.ts` exports no such primitive and no slice here can honestly grant one to the poll loop without inventing a whole write path. What is free and equally true: the ledger already knows how many of a ticket's chunks have settled `done`, so *which chunk is next* is a computation over the file's ordered list and that count. The breakdown therefore **stays immutable after approval**, which is a better fit for [ADR-0014](../../adr/0014-artifact-first-gates.md) than a file the machine edits behind the human's back — an approved artifact that mutates is one the human's approval no longer describes. **The cost, named:** a reader of the file alone cannot see progress; they see it on the ticket, where 23g puts it. **If fvermaut wants the tick in the file, that is a bigger phase and this slice must be told so before it is built.**
...
**[MODIFY]** [`src/daemon/session.test.ts`](../../../src/daemon/session.test.ts) — `gateCommentFor("planning", …)` is asserted at [`:1330`](../../../src/daemon/session.test.ts) and [`:1341`](../../../src/daemon/session.test.ts) and goes red the moment `planning`'s `GATED` row is removed; plus the new assertions below.
...
### 23c — A chore keeps its gate

**[MODIFY]** [`src/daemon/pipeline.ts`](../../../src/daemon/pipeline.ts) — `routeAfterTriage`'s `case "chore"` ([`:420`](../../../src/daemon/pipeline.ts)) advances to `"breakdown"` rather than `"planning"`.
**[MODIFY]** [`src/daemon/pipeline.test.ts`](../../../src/daemon/pipeline.test.ts) — the routing assertions for `chore`.

**Why this slice exists, and it is a regression fix rather than a feature.** A chore skips `requirements` entirely — `routeAfterTriage` sends `triage:chore` straight to `planning` — so **the plan gate is today the single human touch a chore gets**. 23b makes `planning` ungated. Without this slice a chore runs triage → planning → execution → a pull request with **no human gate anywhere before that pull request**. Nothing ships unreviewed, because merging is still a human's act; but the work would be *built* before anyone was asked, and that is a change nobody chose. [ADR-0030](../../adr/0030-the-breakdown-is-a-stage-and-chunk-zero-merges-without-a-pull-request.md)'s consequences name this and say the implementing phase must either route chores through `breakdown` or say plainly that a chore's judgement moves to its pull request.

**A one-chunk breakdown for a small chore is the expected shape and must read naturally.** The artifact is not required to be plural: "one piece — do the thing" is a complete, honest breakdown, and the gate comment over it asks the same question a plan gate used to. **If it reads absurdly on a real chore, that is a finding for 23i, not a licence to skip the gate.**

> **⚠ This slice wants fvermaut's ruling at approval time.** The alternative is genuine: **a chore's judgement moves to its pull request**, `routeAfterTriage` is left alone, and a chore is the one path with no pre-build gate — which is defensible for work triage has already classified as small. Routing chores through `breakdown` costs a gate on every chore, including ones where the answer is obviously yes. **Approving this phase as written chooses the gate.** Striking 23c is a scope *reduction* and keeps the stamp; the rest of the phase does not depend on it.

**Seams under test (TDD):** `routeAfterTriage` in `pipeline.ts`. Red-green cases:

- **A chore is gated.** Red: `routeAfterTriage("chore")` returns `{ kind: "advance", stage: "planning" }`. Green: it returns `{ kind: "advance", stage: "breakdown" }`.
- **Nothing else moved.** `feature` still goes to `clarification`, `bug` to `feedback`, `question` still finishes with its reason — asserted in the same block, because a switch edited by hand is a switch where the neighbouring arm gets touched.
- **The chore path reaches a gate.** With 23b in place, `waitFor(routeAfterTriage("chore").stage) === "gate"` — the property the slice actually buys, asserted as a property rather than as the name of a stage.

> 23b must be complete. 23c is a two-line change to a file 23b rewrites; sequencing them avoids a conflict for no gain.

#### Agent Validation Steps

```bash
npm test -- src/daemon/pipeline.test.ts
npm run type-check
npm test
```

- [ ] Every arm of `routeAfterTriage` is asserted, not just the one that changed
- [ ] The chore assertion is written as *"a chore reaches a gated stage"* as well as *"a chore reaches `breakdown`"* — the first is the requirement, the second is today's mechanism
- [ ] The handoff records that this slice was flagged for a ruling and what the ruling was, so 23i's report can say which shape was gated
...
**A one-chunk chore, if 23c was approved.** Mark a small fixture chore and confirm its breakdown reads naturally rather than absurdly, and that it is gated. One observation, recorded either way — it is the evidence fvermaut's 23c ruling was made without.
...
23c → 23b             a chore keeps its gate
...
**23c, 23d and 23e are the phase's parallel triple** — all three depend only on 23b and they share exactly one file, `prompts.ts` (23d narrows the delivery instruction; 23e rewrites the planning and execution ones; 23c does not touch it). Per [process.md](../../../process.md) stage 6's "parallel only for slices sharing zero files", **run 23d and 23e sequentially**; 23c is genuinely parallel to both.
...
- **That the derived doneness is what fvermaut wants.** [R22 clause 5](../../specs/prd/prd-02-inversion-of-control.criteria.md#r22--a-ticket-hosts-a-sequence-of-chunks) says a merged chunk is *"marked done in the breakdown"*, and this phase keeps the breakdown immutable and puts progress on the ticket instead. The reasoning is in [23a](#23a--the-breakdown-artifact); the ruling is not this plan's to make.
