...
6. **A conversation-parked run whose ticket is closed by hand lingers in the ledger, and `timone status` keeps asking for it.** Tidying the fixture surfaced this: #19 was closed and unmarked directly on the tracker, and its run stayed `parked` — `timone status` went on saying *"waiting on you: a conversation in your terminal"* for a question that no longer existed. `concludeLastConversation`'s own docblock anticipates the lingering and calls it harmless because a closed ticket leaves `listMarkedTickets` — but harmless is exactly what it is not once the status line is the thing the human reads. **The run ends only on a resolution comment**, so a human who settles something by closing the ticket rather than by answering it leaves a permanent nag. The tidy-up was therefore done through the machinery instead: the ticket was reopened and re-marked, the recommendation from the hand-back was accepted in writing, and the daemon resolved it, closed it and ended the run of its own accord at 16:39. **That is itself a fourth observation of the written path, and the first of one following a hand-back** — the escalation does not poison the thread, and an answer written after it is picked up like any other.

7. **Refreshing a blocked ticket when its blocker resolves is nobody's written job, and it happened anyway.** Raised by fvermaut, who expected the five blocked `ivtrends` tickets to be brought up to date by Timone as a matter of course rather than by hand. The expectation is borne out: when `ivtrends` #5 resolved, the session tending the map struck through #7's `Blocked by` line, wrote an *Unblocked* section saying what #5 settled and what it changes here, **replaced the hand-written "nothing needed yet" block with the real both-paths template**, and the mark went on at 16:29:33 — the daemon parked it a minute later carrying a working `timone takeover ivtrends#7`. **Half of that is guaranteed and half is not.** The park comment comes from `TerminalChannel.open` and is code, so it cannot drift. The body rewrite and the mark are a session choosing well: `timone-wayfind` defines what blocking is and what the frontier is, and nowhere instructs a resolving session to refresh the tickets that named it as a blocker. **The failure case is a blocker closed outside Timone** — settled in the human's head and closed by hand. Then nothing tends the map, the dependent keeps a body saying nothing is needed from them, it is never marked, and an answer written on it sits unread: the exact state this phase exists to end, and one the daemon cannot catch because it has no notion of blocking (finding 2). A paragraph in the skill, not code.
