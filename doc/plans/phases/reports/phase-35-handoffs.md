...
## 35e — `process.md` describes the build that no longer stops

**Built.** `process.md` — Timone's own normative description of the whole process, which the three stage skills are supposed to implement — now agrees with the four prior sub-phases instead of contradicting them. Stage 6's note describes execution recording a departure and carrying on after two failed attempts, and describes the same amendment authority (phase file or requirements register, in place, dated, naming the run) the skill file already carries. Stage 7's note describes the environment gate and the fix-loop exhaustion both recording a departure and letting the pass reach its ordinary completion, instead of routing to a human or filing a new ticket; its list of a verification report's required elements now names a "carried-forward" section pointing at the departures record rather than a "handed-to-the-human" one, matching the rename `timone-verify`'s own report template already carries. Stage 8's entry-gate description is narrowed to what still refuses — no verification report at all, or an owed live gate that has not run — and states plainly that a `failed` or BLOCKED register no longer refuses. The "Gates, conversations and the human" section gained one new bullet naming the build-phase carve-out directly: inside `execution`, `verification` and `delivery` an escalation is now a failure to retry, never a wait, while [ADR-0031](../../../doc/adr/0031-a-handoff-is-a-wait-not-a-failure.md) and [ADR-0033](../../../doc/adr/0033-a-stage-that-cannot-act-on-an-answer-escalates.md) still govern every other stage exactly as before. The "Artifact conventions" list gained a line for `plans/phases/reports/phase-NN-departures.md`, alongside the existing `phase-NN-handoffs.md` line.

**Files touched.**

- `process.md` — seven edits: the stage 6 "Escalation, concretely" sentence; stage 7's BLOCKED-verdict sentence and its fix-loop-exhaustion sentence; a consistency fix to stage 7's "Verification report required elements" list (renaming its "handed-to-the-human" section to "carried-forward", matching 35c's own rename in `timone-verify/SKILL.md`); stage 8's "Entry gate" sentence; a new bullet in "Gates, conversations and the human" naming the ADR-0052 carve-out and which ADRs still govern which stages; and a new line in the "Artifact conventions" fenced block for the departures record.
- `doc/plans/phases/reports/phase-35-handoffs.md` — this section appended (by the orchestrator directly, not the sub-agent — see below).

**Decisions taken inside the slice.** One edit beyond the plan's four literal bullets: the consistency fix to stage 7's required-elements list ("a handed-to-the-human section" → "a carried-forward section, pointing at the departures record"). Not separately named by the plan excerpt, but forced by 35c's own earlier rename of the same concept in `timone-verify/SKILL.md` — leaving `process.md` describing an artifact element under a name the skill it normatively describes no longer uses would itself be the kind of contradiction this whole phase exists to remove. The sub-agent that made these edits was told not to touch `phase-35-handoffs.md` or commit; the orchestrator appended this section directly afterward, since only the orchestrator holds the full picture across all five sub-phases needed to write it accurately.
...
42:[...stage 6 note, inline mention of the departures-record file name...]
81:  plans/phases/reports/phase-NN-departures.md build-stage departures — one dated entry per plan step,
exit: 0
```

Checkboxes: first grep exits 1 with zero matches anywhere in the file — the one pre-existing match (inside stage 7's fix-loop sentence) was removed by that sentence's own edit, and no other location in the file used either phrase to begin with (confirmed by running the same grep against the unmodified file before editing: it matched only that one line). Second grep exits 0, with the departures record referenced both in stage 6's note and in the artifact-conventions block. The new "Gates, conversations and the human" paragraph names both [ADR-0031](../../../doc/adr/0031-a-handoff-is-a-wait-not-a-failure.md) and [ADR-0033](../../../doc/adr/0033-a-stage-that-cannot-act-on-an-answer-escalates.md) by number and states plainly which stages each still governs (`requirements`, `breakdown`, the discovery and conversation stages). `git diff --word-diff` confirmed every edit landed as a surgical insertion/deletion inside the target sentence, with the surrounding paragraph text byte-for-byte unchanged — no other stage's note, the "Stages at a glance" table, stage 9's retired note, and the look gate (ADR-0039, stage 8) are all untouched. `npm run build` (exit 0) and `npx vitest run src/daemon` (1179 passed, 21 files) both re-run clean after this sub-phase, confirming the documentation-only change left the codebase's actual behaviour exactly as 35a left it.

**What phase close must know.** This was the last of the five sub-phases (35a–35e). Every sub-phase's own validation passed; the full daemon suite and the build were re-confirmed green after this sub-phase's documentation-only change, which touched no source or test file. No prior sub-phase's validation was invalidated by a later one — `process.md`, `.claude/skills/timone-execute/SKILL.md`, `.claude/skills/timone-verify/SKILL.md` and `.claude/skills/timone-deliver/SKILL.md` are four disjoint files, and `src/daemon/*` (35a) was never touched again after its own commit. One departure was recorded during the phase, in `phase-35-departures.md`: sub-phase 35b's own edits left three further passages in `timone-execute/SKILL.md` describing the pre-ADR-0052 stop behaviour, found and fixed by the orchestrator directly after the sub-agent's work landed, before that sub-phase's commit.
