...
1. **A public false accusation, and it is [phase 15](../phase-15.md)'s fix meeting its known limit.** The path-containment guardrail posted on `scratch-app` #29: *"the session changed 1 file(s) outside `projects/scratch-app/` — timone.yaml (uncommitted change)"*. **The session had not. The orchestrator had** — running `projects update --introduce-unmarked` minutes earlier and leaving the change uncommitted in the working tree. Phase 15 stopped exactly this for **commits**, by reading the provenance trailer ([ADR-0019](../../../adr/0019-timone-authored-commits-carry-a-provenance-trailer.md)); an **uncommitted working-tree change carries no trailer**, so the check still cannot tell whose it is. This is [phase 14](phase-14-live-gate.md)'s 14g fault reappearing through the one gap its fix does not cover, and **it published on a ticket**, under fvermaut's GitHub account, accusing an innocent session in public. Ranked first because it is the only finding here that writes something false where somebody else can read it.
    ...
    **✏ Answered 2026-08-14 by [ADR-0027](../../../adr/0027-a-guardrail-finding-is-addressed-to-the-session-that-caused-it.md), on fvermaut's two rulings, together with finding 11.** The two are one fault pointing opposite ways — this one shouted something wrong where everyone could see it, that one stated something true where nobody could — and the cause is in neither rule: **a finding's destination was chosen by who drove the session, not by who could act on it.**

    Both rulings, and the second is the one that closes *this* finding:

    - **The session that caused it hears first.** At `Stop`, a new violation is handed back to the session, which may not stop until it has fixed it or said why the finding is wrong. The accused session here would have refuted this one instantly — it never touched `timone.yaml` — and **that is knowledge no sharpening of the rule can reach**, because the rule reads git and the answer was in an uncommitted working tree.
    - **Nothing is ever posted on a client's ticket.** A run's escalation flags the run, which is what `timone status` reads. `postComment` leaves the guardrail path entirely. Timone posts through fvermaut's account, so a guardrail comment arrives on somebody's thread as *his* judgement of their work — and it has now been wrong there twice ([14g](phase-14-live-gate.md), and this).

    **Built the same day**, with the round's three states kept per violation in the parked baseline: told once, escalated once, silent thereafter. **818 tests pass**, including the two-stop round on real git. **The cost is named rather than buried:** `PRD-02` R15 loses its `verified` status, since its second criterion asserted the ticket comment this reverses; a session can now waste a turn arguing with a correct finding; and a session **killed** after being handed one escalates nothing at all, which is a real hole recorded in the ADR rather than engineered around.

    **What this does *not* fix is the attribution itself.** An uncommitted change still carries no trailer, and the rule will still name the wrong session — it now says so to a session that can answer back, instead of to a ticket that cannot.
...
11. **A managed project's work branch was created a second time inside the Timone repository, and every close-out artifact of phases 19 and 20 was then committed onto it.** `timone/29-fixture-map-notes-on-a-to-do-phase-20-ga` exists twice: **correctly in `projects/scratch-app`**, cut at 15:34 by the requirements run and deleted with the rest of the fixture at 16:02 — and **wrongly in `/Users/fvermaut/dev/timone`**, created between 15:52 and 15:54 by a session sitting at the Timone root. The two reflogs are the evidence, and they interleave:
    ...
    **✏ Repaired 2026-08-14, on fvermaut's instruction.** `main` was a strict ancestor of the branch, so the repair was a fast-forward to `60cb4fc`, a branch delete, and one push of all 13 commits. **Nothing merged and nothing conflicted** — the branch held no fixture work at all, only Timone's own, which is why the damage was recoverable.

    **✏ Fixed 2026-08-14, both halves, on fvermaut's instruction.** The instruction and the check were wrong in the same place, so both moved:

    - **`checkoutBlock`** now names the repository wherever a branch is named — *"`<project>`'s git repository is `projects/<project>/`, and every git command you run belongs there"*, with the reason attached: at the timone root a bare `git` acts on Timone's own repo. It is appended in `stagePrompt`, beside the provenance block and for the same reason — **a per-stage copy is a per-stage chance to forget it** — and written into `approvalRecordPrompt` by hand, because that prompt is built outside `stagePrompt` and inherits none of the shared blocks. That inheritance gap is why the shortest prompt in the file was the one that did this.
    - **`checkBranchPlacement`** is a new rule: any branch named `timone/…` in the workspace is a project's work branch in the wrong repository. It is decidable on the name alone — Timone is not a managed project, so no run ever targets it and no work branch is ever named for it — and it needs no `target`, because the session that made the branch was recording an approval and the session that paid for it was interactive. When the run's target is known the violation names the checkout it belonged in; otherwise it names `projects/…`. It reports whatever is already stranded on the branch, which is the part that says how urgent it is.

    **Seventeen tests, one of them on real git**: the event itself replayed — `git checkout -b timone/29-…` at the timone root, a commit on it — and the rule fires, prints and journals. 810 pass, type-check clean.

    **✏ And the surfacing was settled the same day, for this finding and finding 1 together** — [ADR-0027](../../../adr/0027-a-guardrail-finding-is-addressed-to-the-session-that-caused-it.md). A branch cut in the wrong repository is now handed to the session that cut it, while that session still exists and can move it. See finding 1 for the ruling and what it costs.
