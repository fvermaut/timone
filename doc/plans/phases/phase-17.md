...
> **Status:** **Complete.** Executed 2026-08-08, all four sub-phases. Report: [reports/phase-17-complete.md](reports/phase-17-complete.md); gate evidence: [reports/phase-17-live-gate.md](reports/phase-17-live-gate.md). **R18 `verified`; R17 stays `draft` on the token half**, exactly as this plan required of itself. **17c's human gate is outstanding** — fvermaut instructed the gate be run without him, so it was, and the one judgement made on his behalf (lifting the overnight prohibition) is named in [STATUS.md](../../../STATUS.md) for him to reverse in a word. **17c was also run far more cheaply than this plan specified**, after he called the specified gate overkill and was right: steps 1–3 read nothing but the ledger's own timestamps, so they needed no agent session, no scheduling and no money. Steps 4–5 do, and are not discharged.

> **Approved for execution by fvermaut 2026-08-08**, immediately after [phase 16](phase-16.md) closed — the moment this plan was written for. Planned 2026-08-08 at the moment of the split, so the decision would not evaporate in between; parked until previews landed; hand-planned, as all Timone-self phases are. Approval was gated on this file per [ADR-0014](../../adr/0014-artifact-first-gates.md).

> **Re-checked against `main` at approval, as this plan required of itself.** Phase 16 added a top-level `previews` key to the same `stateSchema` 17a extends, and it landed first — so the two do not collide. What 17a must know, concretely: `stateSchema` is a `z.strictObject` carrying `version`, `runs` **and now `previews`**; `RunStore` has gained `previewRecord`, `previewsFor`, `recordPreview` and `forgetPreview` beside the run methods; `poll.ts` now calls `reconcilePreviews` per project, **after** `reclaimStale` and `pollProject`, so a witness taken once per cycle before any reclaim is still taken before everything that matters. **Nothing in this plan changes as a result** — it is confirmed, not amended.
