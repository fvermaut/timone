...
3. A second ticket queued behind that same parked occupier: after the poll that cancels the parked run, the queued run is still `queued` (nothing spawns in the same cycle, matching the `picked-up` case's own behaviour); on the **next** poll, `store.promoteQueue` picks it up and it is started. This is the exact sequence [timone#99](https://github.com/fvermaut/timone/issues/99) reported by hand (`timone cancel`, then the next cycle promoted #91) — reproduced here as a daemon behaviour, needing no hand-run command.
