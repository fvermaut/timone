...
**When the work is un-anchored, derive the regression set and say what it does not cover.** "Protected by the regression set" is a promise, and it is not always worth much: the set is *derived* — every criterion with priority MUST, verify-via `api`, and status `verified`, ✏ **then narrowed to those whose declared dependencies this phase's diff touches** ([ADR-0051](../../../doc/adr/0051-timone-verifies-itself-by-live-gate-and-a-regression-set-is-narrowed-by-what-it-depends-on.md) D4) — so a `browser` or `live` criterion is outside it however solid it looks, a criterion stage 9 has just marked `revised` has left it, and a criterion whose `Depends-on` this phase does not touch is not watching this phase either. A `live` criterion is worth naming separately in the Goal Description when the phase touches what it depends on: that phase **owes a live gate before delivery**, and planning is where that cost is seen rather than discovered at stage 8. Compute it from the register at planning time, state it in the Goal Description, and name the behaviour this phase could break that **no criterion in that set watches**. Where the real protection turns out to be a single test, make that test a hard gate in the validation steps rather than a courtesy. A refinement is by construction work where nothing is currently failing — which means nothing is currently watching either.
...
- **A user-facing screen gets a shell slice first** — ✏ 2026-08-19 ([ADR-0038](../../../doc/adr/0038-a-screens-shell-is-built-before-its-behaviours.md)). When the phase builds or substantially reworks a screen, its **first** sub-phase is the screen itself: the stylesheet (from the kept prototype per [ADR-0037](../../../doc/adr/0037-a-prototype-that-settles-a-look-is-kept-and-only-its-presentation-crosses.md), or `doc/design.md` where no prototype settled this screen), the layout and regions, the density, how a figure is set, how a column is sized — plus enough static content to render, so it is a screen a person can look at rather than a stylesheet nobody has seen. It carries the accessibility obligations from its first commit (reflow, focus treatment, non-colour carriers), because those are layout decisions and retrofitting them is what forces a later slice to restyle. Every other slice depends on it.
  - **Name the reference in the slice**, by branch or by file path. The shell's validation is a comparison against it and cannot run without it.
  - **Later slices fill the shell in and may not restyle.** Say so in the phase file. A slice that finds the shell wrong amends the shell — a marked amendment naming what moved and why — rather than styling around it locally; local overrides accumulating across contexts are individually reasonable and collectively the defect.
  - **A phase with no screen has no shell slice** and is not defective for it. Adding one column to an existing board is not "substantially reworks"; introducing a screen, or changing its layout, is. Wrongly adding one costs a small slice. Wrongly omitting one is `ivtrends` phase 05.
- **Name every screen the phase changes** — ✏ 2026-09-22 ([ADR-0057](../../../doc/adr/0057-every-screen-a-phase-changes-is-looked-at-and-its-figures-read-on-the-preview-data.md)). Fill the header's **Screens changed** line with the address of every screen whose visible content any slice adds to or alters, and the reference each is compared with — the kept prototype, or `doc/design.md`. This is wider than the shell-slice rule above: a new section, a new column or a new label on an existing screen counts. Execution looks at each of them before the phase closes, and verification reads their figures; neither can run on a screen the line leaves out. Write *none* only when no slice changes anything a person sees. `ivtrends` phase 36 added a whole section to the positions screen, built no shell slice, and nothing looked at it.
- **A requirement that shows something new carries a browser line** — ✏ 2026-09-22 ([ADR-0057](../../../doc/adr/0057-every-screen-a-phase-changes-is-looked-at-and-its-figures-read-on-the-preview-data.md)). For every claimed requirement whose outcome puts something new on a screen, check that the register gives it at least one clause marked `(browser)`: what the person sees and where it sits. When one has none, **do not stop**: add the clause to the register as a marked amendment (`✏ <date> — added at planning, phase NN, per ADR-0057`), written from the requirement's own words, and record it in the phase's departures record. The requirement keeps its channel; the clause is checked in a browser.
...
> **Screens changed:** <`/path` — compared with <the kept prototype's branch, or `doc/design.md`>, one per screen; or "none — no slice changes anything a person sees">
<✏ 2026-09-22 ([ADR-0057](../../../doc/adr/0057-every-screen-a-phase-changes-is-looked-at-and-its-figures-read-on-the-preview-data.md)). Stage 6 looks at every screen listed here before the phase closes, and stage 7 reads their figures on the preview's data. Never omit the line.>
