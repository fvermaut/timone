...
**Past those two gates, this stage asks a person for nothing, and ends only at delivery.** ✏ Added 2026-09-11, after the fault below. Gate 1 and gate 2 fire *before* any verifying happens, and what they mean is that the work never reached this stage at all. Once you are past them you are inside the build, and [ADR-0052](../../../doc/adr/0052-a-run-that-enters-the-build-ends-at-its-pull-request.md) is absolute there: between the human's agreement to the piece and its pull request, no step stops on a person, because the question itself is the defect. Every outcome this stage can produce that is short of a clean pass — a check that could not run, a standing regression criterion BLOCKED, a script only a person can perform, a criterion left `failed` after both fix loops, an environment that never came up — is **recorded in the report and in the departures record, and carried into the pull request by stage 8**. None of them is a reason to post a question, and none of them is a reason to end anywhere but `timone-deliver`. The pull request is where the human already is, and it is the only place this run is allowed to put a decision in front of them.

**This is written because it was got wrong.** On 2026-09-11 an `ivtrends` pass did its whole job correctly — report, register flips, departures record, four claimed criteria checked — and then posted *"two things stop it being signed off"* and parked the run on fvermaut, over nine BLOCKED standing checks and two manual scripts. Neither is a stop. Three sentences in this file said otherwise and have been corrected; this paragraph is the rule they were missing.

**And it was got wrong again, so the machinery no longer waits for you.** ✏ Added 2026-09-12 ([ADR-0056](../../../doc/adr/0056-a-build-stages-question-rides-to-the-pull-request.md)). On 2026-09-12 an `ivtrends` pass asked the same class of question again — on a run whose instructions already said all of the above — and asked it a second time after being restarted. So the daemon now reads a stop here as a finish: whatever you post, the run walks on to delivery, and your words are handed to stage 8 to put in the pull request. **Nothing you write here holds the work, and nobody is coming to answer it.** A question posted instead of a report is therefore not a stop, it is a paragraph of your own prose landing in a pull request out of context. Write the report, put what is outstanding in it, and let delivery carry it.
...
- **`live`** ✏ Added 2026-09-04 ([ADR-0051](../../../doc/adr/0051-timone-verifies-itself-by-live-gate-and-a-regression-set-is-narrowed-by-what-it-depends-on.md) D1) — what only a **supervised run against real infrastructure** can observe: a real daemon, real credentials, a real forge, real containers, real managed projects. Reported as **LIVE-GATE**.
  ...
  **It is not `BLOCKED` either.** BLOCKED means the check could not run at all. A `live` criterion is not expected to run here, so it is not one that failed to run: it reports its last gate and the pass continues. ✏ 2026-09-11: this line used to end *"and the gate stops"*, which stopped being true at [ADR-0052](../../../doc/adr/0052-a-run-that-enters-the-build-ends-at-its-pull-request.md). Nothing here stops.
...
**Verdicts:** PASS / FAIL / HUMAN-CHECK / LIVE-GATE / BLOCKED. **REGRESSION** is the FAIL variant reserved for a criterion that entered the pass already `verified` — same failure class, counts against the gate's zero-regressions clause, consumes the same fix loop; the distinct label exists because shipped behaviour broke and stage 9 reads it differently. **BLOCKED** asserts nothing about behaviour: register untouched, no loop consumed — and ✏ since [ADR-0052](../../../doc/adr/0052-a-run-that-enters-the-build-ends-at-its-pull-request.md) **nobody routed to and nothing stopped**. One entry goes in the departures record, the report says what could not be checked and why, and the pass carries on with the rest of its scope. A BLOCKED criterion is **not** an unresolved regression: a regression is a criterion that entered the pass `verified` and was observed to be wrong, and BLOCKED observed nothing. Confusing the two is what parked `ivtrends` #89 on a person; the line above used to read *"gate blocked, human routed"*, and that is what it was read to mean. **LIVE-GATE** is the `live` channel's only outcome: register untouched, gate **not** blocked, no loop consumed — it reports which gate last observed the criterion and whether this phase owes a fresh one. It is deliberately not BLOCKED: a criterion that was never going to run here has not failed to run.
...
**The closing gate, restated from the spec:** all MUST criteria PASS, HUMAN-CHECK or LIVE-GATE, zero regressions, within 2 verify-fix loops.
...
Verification observes and reports. It never fixes with its own hands, never merges, and never opens a PR. It never asks the human for a decision, an answer or a waiver — that is stage 8's pull request, and this stage's job is to give it everything it needs to be read. Stop here.
