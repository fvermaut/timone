...
4. That brief-fix-reverify cycle is **one loop**. **Max 2 loops after the initial pass.** At exhaustion: remaining failures recorded with evidence, their register lines flipped to `failed`, and the work goes to the human via stage 9 (`/timone-improve <project> phase-NN verification, loop exhaustion`) — the report's handed-to-the-human section says exactly what remains and why.
