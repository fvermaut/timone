...
**1 — Agreement gate.** ✏ Revised 2026-08-15 ([ADR-0030](../../../doc/adr/0030-the-breakdown-is-a-stage-and-chunk-zero-merges-without-a-pull-request.md) D1). **What execution refuses to start without is agreement, and what carries agreement is the breakdown — the list of pieces an initiative is being built in, at `doc/plans/breakdowns/ticket-NN.md` — not the phase file.** So for ticket-driven work: read that file, and build only when its `Status` line reads `Approved by <who> <date> — N pieces` **and** the piece this phase plans is one the list names. A breakdown still stamped `Awaiting approval` is not executable; neither is an approved one whose numbered list has since grown past the count in its own stamp, because the piece you were handed may be one nobody has seen. Route to the **human** when no breakdown was ever approved — re-planning cannot manufacture a missing approval — and to **`timone-plan`** when a re-proposal is what left the approval stale. Never execute "just the pieces that were approved" out of a list under re-proposal.

**Two shapes of work have no breakdown by design, and their gate is not this one.** A chore or technical enabler triage routed to planning meets no gate before its pull request ([ADR-0030](../../../doc/adr/0030-the-breakdown-is-a-stage-and-chunk-zero-merges-without-a-pull-request.md) D3), and hand-run work with no driving ticket was agreed by the human who invoked you. Both build; neither is refused for an absent breakdown. What still refuses is an initiative that *has* a driving ticket and no approved breakdown — there the absence means stage 5's gate never closed, and that routes to the human.
...
3. The **file list** the slice may create or modify, plus two implicit grants no plan lists and every slice needs: `doc/plans/phases/reports/phase-NN-handoffs.md`, which every slice appends to — **this grant outranks a plan checkbox that would forbid it** (a slice asserting "no process artifact under `doc/` was modified" cannot mean its own mandated handoff; resolve it that way, state the interpretation in the handoff, and do not escalate a whole slice over the wording) — and **the test files for its declared seams** — the TDD loop cannot write a failing test into a file it is not allowed to create. Nothing else is implicit. In particular, a slice may never touch a config file it was not granted, even when that config is what makes its own validation fail; that is gate 3.
...
**Commit only after the slice has returned.** Not while it is still working — a mid-flight commit snapshots a half-written handoff section and puts the commit boundary in the wrong place, so `git log` stops answering which slices landed. If a slice is still producing evidence, wait for it.

**Never edit code, tests, or comments to satisfy an assertion you believe is wrong.** A validation step that a correct implementation cannot pass is a plan defect — gate 3 — and the fix is an amendment, not a change to the work. The tempting move is small and specific: reword a comment so a grep stops matching it, rename a symbol a probe forbids, delete an explanatory line. Every one of those makes the artifact worse to satisfy a broken check, and it hides the defect from the next slice, which meets it again. If you have already done it, say so plainly in the handoff and record the defect anyway — an openly-recorded workaround is far better than a silent one, but it is still a workaround, and the amendment is still owed.
...
**Re-run every prior slice's validation — after any slice that mutates shared state, and again before the phase closes.** The per-slice gate is necessary and not sufficient: a later slice can pass its own block while breaking an earlier slice's committed tests, because each block runs only the commands its own slice cared about. A seed that adds a row, a migration that changes a default, a config change — any of these can turn an earlier green suite red without any command in the current slice noticing. Waiting for the close means a long phase carries the break through every intervening slice before anyone sees it, so re-check as soon as a slice's deliverable touches state an earlier slice observed. If a prior validation now fails, the phase does **not** close: that is gate 3, and the fix belongs to a plan amendment granting the file to a slice, never to you patching it quietly.

**A checkbox the plan intends a later slice to falsify is superseded, not failed.** Plans legitimately assert a state that a subsequent slice is scheduled to change — "the schema declares no models (the model is NNb's deliverable)" is true when written and false by design once NNb lands, and its own text says so. Read literally, re-running it would block every close. Scope the re-run to checkboxes still meaningful at close: one whose own wording assigns the contradicting deliverable to a later slice is superseded, and you say so rather than either failing it or quietly skipping it. A prior checkbox that nothing in the plan predicted would change is a real regression.

**Order the re-runs by their side effects, and restore shared state between them.** Prior validation blocks are a destructively-interacting sequence, not a set: one may reset a database, another may truncate a table, a third may then assert that a row exists. Running them in written order can manufacture a failure that says nothing about the code. Derive an order from what each block does to state outside the repo, restore that state between blocks where needed, and report the order you used so the result is reproducible.
...
2. Flip the phase file's `Status` line — replacing it, since the line has one state at a time:
   ...
   ✏ Revised 2026-08-15 — the line it replaces carries no approval trace any more, because a phase file no longer has one. What authorised the work still belongs in the report's **Plan** line, which is why that element is required there: for a ticket-driven piece that is the approved breakdown, named with its `Approved by <who> <date> — N pieces` stamp; for a chore or hand-run phase there is none, and the line says so.
