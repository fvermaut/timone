...
| Falsified-by | a test, type or probe, or absent | What would go red if the criterion stopped holding. **Required before a MUST whose claim is universal can ever reach `verified`** ([ADR-0055](../../../doc/adr/0055-a-universal-claim-is-not-established-by-watching.md)): a claim that a thing *never* or *always* happens is about every path, and a live gate establishes the paths it walked. Write one whenever you write a universal — that is the moment it is cheapest to decide what could disprove it |
