...
- **Preserve what you replace whenever another committed artifact cites it** — and always when a human wrote it. A dated grant or ratification records a decision someone made, and rewriting it silently erases that the decision was once justified differently; but the broader and commoner reason is citation: reports name entries by line, and deleting the text orphans a pointer in an artifact you cannot edit. Quote the old wording inside the marker and say why it was wrong. **Withdraw in place rather than delete**, so a reader arriving from the citation still lands on something — and word the withdrawal so the section around it stays literally true.
